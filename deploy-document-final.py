...
def main():
    ...
    try:
        ...
        # Try to navigate to css directory
        try:
            ftp.cwd('../css')
            print("✓ Found css directory")
            
            # Upload CSS files
            print("\nUploading CSS files...")
            upload_file(ftp, 'app/css/member-profile.css', 'member-profile.css')
        except:
            print("⚠ CSS directory not found, attempting to create it...")
            try:
                ftp.cwd('..')
                ftp.mkd('css')
                ftp.cwd('css')
                ...
                print("⚠ Could not create/access css directory, skipping CSS upload")
