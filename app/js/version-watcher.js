(function() {
    ...
    const VERSION_URL = window.__PYEBWA_VERSION_URL__ || '/app/version.json';
    ...
    function showUpdatePrompt(nextBuildId) {
        ...
        prompt.querySelector('.version-update-banner__action').addEventListener('click', async () => {
            lastSeenBuildId = nextBuildId;

            if (window.swManager && typeof window.swManager.checkForUpdates === 'function') {
                await window.swManager.checkForUpdates();
            }

            if (window.swManager && typeof window.swManager.applyUpdate === 'function' && window.swManager.registration?.waiting) {
